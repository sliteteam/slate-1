      ...
      case 'insertText': {
        ...
        if (Path.equals(targetRange.anchor.path, targetRange.focus.path)) {
          ...
          if (
            ...
            insertPositionHint &&
            type === 'insertCompositionText' &&
            insertPositionHint.text.endsWith(text) &&
            insertPositionHint.start === start.offset - 1 &&
            insertPositionHint.end === end.offset
          ) {
            scheduleFlush()
            debug('ignore swiftKey replace')
            insertPositionHint = false
            return
          }
          ...
          if (type === 'insertText' && insertPositionHint === null) {
